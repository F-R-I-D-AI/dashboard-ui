...
// 백엔드 라우터 설정에 따라 /v1/detect_faces가 맞으므로 이 주소 사용
...
export default function Home() {
  ...
  async function requestFaceDetection(file: File) {
    ...
    // [확인] BE api/routes/detect.py의 prefix가 /v1 이므로 여기가 맞습니다.
  ...
  async function cropFacesFromAfterImage(afterImgUrl: string, faces: FaceBox[], detectionMeta: {width: number; height: number}) {
    const img = new Image();
    ...
    const scaleX = img.width  / detectionMeta.width;
    const scaleY = img.height / detectionMeta.height;
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12 space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-10 items-start">
        ...
        {/* 오른쪽 — 업로드 / 버튼 패널 */}
        <div className="rounded-xl border border-[var(--border)] p-6 bg-[var(--panel)]">
          <h2 className="font-semibold text-lg mb-4">이미지 업로드</h2>
            
          <label
            className="block px-4 py-3 text-center bg-[var(--accent)]/15 text-[var(--accent)] rounded-md cursor-pointer border border-[var(--accent)]/40"
          >
            파일 선택하기
            <input
              type="file"
